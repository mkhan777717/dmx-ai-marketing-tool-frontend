...
export const AnalyticsService = {
  getDashboard(workspaceId: string) {
    return api.get<ApiResponse<DashboardOverviewResponse> | DashboardOverviewResponse>(
      ENDPOINTS.ANALYTICS_DASHBOARD(workspaceId)
    );
  },

  getOverview(workspaceId: string, snapshotType = "daily") {
    return api.get<ApiResponse<AnalyticsSnapshotResponse> | AnalyticsSnapshotResponse>(
  ...
  getCampaignAnalytics(
    ...
    return api.get<ApiResponse<CampaignAnalyticsResponse[]> | CampaignAnalyticsResponse[]>(
  ...
  getAIUsage(workspaceId: string, params?: { skip?: number; limit?: number }) {
    return api.get<ApiResponse<AIUsageResponse[]> | AIUsageResponse[]>(
